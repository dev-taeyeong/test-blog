...
배열을 선언하는 이유는 **배열 값이 한개의 값이 아닌 여러개의 데이터 값을 동시에 가질 수 있기 때문** 입니다. 그래서 데이터의 집합을 저장하기 위한 용도로 사용합니다.
...
do ... while 문은 코드 블록을 먼저 실행하고 조건식을 평가합니다. 따라서 **코드 블록은 무조건 한 번 이상 실행** 됩니다.
...
배열은 연관있는 데이터를 연속적으로 나열한 자료구조입니다. 보통 연관있는 데이터에는 동일한 작업을 해줄텐데요, 이렇게 연속적으로 나열된 데이터에 동일한 작업을 해 주는데는 반복문이 최적화 되어있기 때문에 함께 사용되는 것 같습니다.
